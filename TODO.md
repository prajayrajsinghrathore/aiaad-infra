...
- [ ] Validate Istio Gateway and ingress routing for applications.
- [ ] Set up production-grade PostgreSQL (if not using local deployment).
- [ ] Actually deploy the Istio routing VirtualService (aiaad-platform-routing) that failed during the smoke test (currently only applied with --dry-run=client).
